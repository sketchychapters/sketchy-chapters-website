...
async function markEnrollmentStatus(studentUid, enrollmentId, newStatus) {
  ...
  if (completedCourseName) {
    ...
    await db.collection("certificates").doc(certId).set({
      ...
      completionDate: completedEndDate,
    ...
    await ref.update({
      certificates: firebase.firestore.FieldValue.arrayUnion({
        id: certId,
        courseName: completedCourseName,
        url: "certificate.html?id=" + certId,
        dateIssued: new Date().toISOString(),
        autoGenerated: true
      })
    });

    await addNotification(studentUid, `🎓 شهادتك الإلكترونية لدورة "${completedCourseName}" جاهزة! افتح حسابك لعرضها.`);
  }
}
